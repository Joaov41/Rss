...
/// Displays transcript-style `[mm:ss]` citations as controls. The answer
/// renderer keeps normal text selectable elsewhere, while podcast citations
/// explicitly seek the single app-shell player and resume playback.
struct PodcastTimestampCitationView: View {
    let text: String
    @ObservedObject var player: PodcastEpisodePlayerController
    ...
    var body: some View {
        Text(linkedText)
            ...
            .environment(\.openURL, OpenURLAction { url in
                guard url.scheme == "rss-podcast-time",
                      let seconds = TimeInterval(url.host ?? "") else {
                    return .systemAction
                }
    ...
    private var linkedText: AttributedString {
        ...
        guard let regex = try? NSRegularExpression(
            pattern: #"\[(?:\d{1,2}:)?\d{1,2}:\d{2}(?:[–-](?:\d{1,2}:)?\d{1,2}:\d{2})?\]"#
        ) else {
            return attributed
        }
        ...
        for match in regex.matches(in: text, range: fullRange).reversed() {
            guard let sourceRange = Range(match.range, in: text),
                  let lower = AttributedString.Index(sourceRange.lowerBound, within: attributed),
                  let upper = AttributedString.Index(sourceRange.upperBound, within: attributed) else {
                continue
            }
            let label = String(text[sourceRange])
            guard let seconds = Self.seconds(in: label),
                  let url = URL(string: "rss-podcast-time://\(seconds)") else {
                continue
            }
    ...
    private static func seconds(in citation: String) -> TimeInterval? {
        let trimmed = citation.trimmingCharacters(in: CharacterSet(charactersIn: "[]"))
        let first = trimmed.split(whereSeparator: { $0 == "–" || $0 == "-" }).first.map(String.init) ?? trimmed
        let components = first.split(separator: ":").compactMap { Double($0) }
