...
actor PodcastEpisodeTranscriptService {
    ...
    private func generatedTranscript(
        ...
        // Speech analysis is deliberately utility priority. A long episode can
        // produce a sustained stream of partial results, and transcription
        // should remain responsive to navigation and playback work.
    ...
    private static func collectSegments(
        ...
        for try await result in transcriber.results {
            ...
            if let end, let audioDuration, audioDuration > 0 {
                ...
                let now = Date()
                // SpeechTranscriber emits many tentative results for the same
                // portion of audio. Coalesce those updates to at most 5 Hz and
                // never emit the same displayed percentage twice.
                if percentage != lastReportedPercentage,
                   now.timeIntervalSince(lastReportedAt) >= 0.2 {
        ...
        if audioDuration != nil, lastReportedPercentage != 100 {
            await onProgress?(1)
        }
        return segments
    }
