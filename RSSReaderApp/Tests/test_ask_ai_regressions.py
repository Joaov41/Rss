...
class MacAskAIRegressionTests(unittest.TestCase):
    ...
    def test_web_ai_handoff_gemini_confirms_submission_before_success(self):
        ...
        self.assertIn("activateAction(sendButton, true)", web_view)
        self.assertIn("__codexWebAIPreparedPrompt", web_view)
        self.assertIn("__codexWebAISubmissionPending", web_view)
        self.assertNotIn("__codexGeminiSubmissionPending", web_view)
        self.assertIn("assistantTurnCount() > startingTurns", web_view)
