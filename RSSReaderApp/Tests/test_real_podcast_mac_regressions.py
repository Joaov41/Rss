...
class RealPodcastMacRegressionTests(unittest.TestCase):
    ...
    def test_normal_rss_articles_do_not_use_the_feed_logo_as_item_artwork(self):
        feed = read("RSSReaderApp/Services/FeedService.swift")
        rss = feed.split("// MARK: - RSS", 1)[1].split("// MARK: - Atom", 1)[0]
        self.assertIn("var articleImageURL = podcastAudioURL.flatMap", rss)
        podcast_artwork = rss.split(
            "var articleImageURL = podcastAudioURL.flatMap", 1
        )[1].split("if articleImageURL == nil", 1)[0]
        self.assertIn("feed.image?.url", podcast_artwork)
        self.assertLess(rss.index("classification.shouldUseAsArtwork"), rss.index("mediaContents"))
        self.assertLess(rss.index("mediaContents"), rss.index("extractFirstImageURL"))
