...
final class LLMRunner {
    ...
    private let reservedCompletionTokens = 256
    private let attachmentTrimNotice = "\n\n[Attachment truncated to fit the available context window.]"
    ...
    private func loadMonolithic(url: URL, folder: URL, config: MLModelConfiguration) async throws {
        ...
        if let model {
            applyRuntimeShapeOverrides(from: model)
        }
    ...
    private func loadChunked(folder: URL, config mlConfig: MLModelConfiguration) async throws {
        ...
        if let chunk1 {
            applyRuntimeShapeOverrides(from: chunk1)
            try validateChunkShapes()
            if let decodeCtx = chunkedDecodeContextLimit() {
                contextLength = decodeCtx
            }
            if let decodeSliding = chunkedDecodeSlidingWindow() {
                slidingWindow = decodeSliding
            }
            if let prefillBatch = prefillBatchLimit() {
                prefillN = prefillBatch
            }
            let prefillCtx = prefillBatchLimit()
            let c1 = maskSequenceLength(for: chunk1, inputName: "causal_mask_full") ?? -1
            let c2 = chunk2.flatMap { maskSequenceLength(for: $0, inputName: "causal_mask_full") } ?? -1
            let c3 = chunk3.flatMap { maskSequenceLength(for: $0, inputName: "causal_mask_full") } ?? -1
            let c4 = chunk4.flatMap { maskSequenceLength(for: $0, inputName: "causal_mask_full") } ?? -1
            print("[Load] Runtime shapes: full masks c1=\(c1) c2=\(c2) c3=\(c3) c4=\(c4), sliding=\(chunkedDecodeSlidingWindow() ?? -1), prefill batch=\(prefillCtx ?? -1), effective ctx=\(contextLength)")
        }
    ...
    private func applyRuntimeShapeOverrides(from model: MLModel) {
        let inputs = model.modelDescription.inputDescriptionsByName
        if let inferredContext = maskSequenceLength(for: model, inputName: "causal_mask")
            ?? maskSequenceLength(for: model, inputName: "causal_mask_full") {
            contextLength = inferredContext
        } else if let shape = inputs["K_full_in"]?.multiArrayConstraint?.shape, shape.count >= 3 {
            contextLength = shape[2].intValue
        }

        if let inferredSliding = maskSequenceLength(for: model, inputName: "causal_mask_sliding") {
            slidingWindow = inferredSliding
        } else if let shape = inputs["K_sliding_in"]?.multiArrayConstraint?.shape, shape.count >= 3 {
            slidingWindow = shape[2].intValue
        }
    }

    private func maskSequenceLength(for model: MLModel, inputName: String) -> Int? {
        guard
            let shape = model.modelDescription.inputDescriptionsByName[inputName]?.multiArrayConstraint?.shape,
            shape.count == 4
        else {
            return nil
        }
        return max(shape[2].intValue, shape[3].intValue)
    }

    private func kvCacheSequenceLength(for model: MLModel, inputName: String) -> Int? {
        guard
            let shape = model.modelDescription.inputDescriptionsByName[inputName]?.multiArrayConstraint?.shape,
            shape.count >= 3
        else {
    ...
    private func chunkedDecodeContextLimit() -> Int? {
        [chunk1, chunk2, chunk3, chunk4]
            .compactMap { $0 }
            .compactMap { maskSequenceLength(for: $0, inputName: "causal_mask_full") }
            .min()
    }

    private func chunkedDecodeSlidingWindow() -> Int? {
        [chunk1, chunk2, chunk3, chunk4]
            .compactMap { $0 }
            .compactMap { maskSequenceLength(for: $0, inputName: "causal_mask_sliding") }
            .min()
    }

    private func prefillBatchLimit() -> Int? {
        [prefillChunk1, prefillChunk2, prefillChunk3, prefillChunk4]
            .compactMap { $0 }
            .compactMap { maskSequenceLength(for: $0, inputName: "causal_mask") }
            .min()
    }

    private func validateChunkShapes() throws {
        let decodeCtxs = [chunk1, chunk2, chunk3, chunk4]
            .compactMap { $0 }
            .compactMap { maskSequenceLength(for: $0, inputName: "causal_mask_full") }
        let decodeSliding = [chunk1, chunk2, chunk3, chunk4]
            .compactMap { $0 }
            .compactMap { maskSequenceLength(for: $0, inputName: "causal_mask_sliding") }
        let prefillCtxs = [prefillChunk1, prefillChunk2, prefillChunk3, prefillChunk4]
            .compactMap { $0 }
            .compactMap { maskSequenceLength(for: $0, inputName: "causal_mask") }

        let decodeCtxSet = Set(decodeCtxs)
        let decodeSlidingSet = Set(decodeSliding)
        let prefillCtxSet = Set(prefillCtxs)

        guard decodeCtxSet.count <= 1 else {
            throw NSError(
                domain: "LLMRunner",
                code: 20,
                userInfo: [NSLocalizedDescriptionKey:
                    "Mixed decode chunk context lengths detected: \(decodeCtxs). Delete the model and download it again."]
            )
        }
        guard decodeSlidingSet.count <= 1 else {
            throw NSError(
                domain: "LLMRunner",
                code: 21,
                userInfo: [NSLocalizedDescriptionKey:
                    "Mixed decode sliding-window sizes detected: \(decodeSliding). Delete the model and download it again."]
            )
        }
        guard prefillCtxSet.count <= 1 else {
            throw NSError(
                domain: "LLMRunner",
                code: 22,
                userInfo: [NSLocalizedDescriptionKey:
                    "Mixed prefill chunk context lengths detected: \(prefillCtxs). Delete the model and download it again."]
            )
        }
    }

    private func makeChunkMasks(for model: MLModel, position: Int) throws -> (
        full: MLMultiArray, sliding: MLMultiArray, update: MLMultiArray
    ) {
        let fullLen = maskSequenceLength(for: model, inputName: "causal_mask_full") ?? contextLength
        let slidingLen = maskSequenceLength(for: model, inputName: "causal_mask_sliding") ?? slidingWindow
        let updateLen = maskSequenceLength(for: model, inputName: "update_mask") ?? fullLen
        let full = try makeCausalMask(position: position, contextLength: fullLen)
        let sliding = try makeSlidingCausalMask(position: position, W: slidingLen)
        let update = try makeUpdateMask(position: position, contextLength: updateLen)
        return (full: full, sliding: sliding, update: update)
    }
    ...
    private func allocateSWAKV(maxHd: Int) throws {
        let ctx1 = chunk1.flatMap { kvCacheSequenceLength(for: $0, inputName: "K_full_in") } ?? contextLength
        let ctx2 = chunk2.flatMap { kvCacheSequenceLength(for: $0, inputName: "K_full_in") } ?? contextLength
        let w1 = chunk1.flatMap { kvCacheSequenceLength(for: $0, inputName: "K_sliding_in") } ?? slidingWindow
        let w2 = chunk2.flatMap { kvCacheSequenceLength(for: $0, inputName: "K_sliding_in") } ?? slidingWindow
        ...
        kSliding1 = try ioSurfaceArray(slots: 7, seqLen: w1)
        vSliding1 = try ioSurfaceArray(slots: 7, seqLen: w1)
        kFull1 = try ioSurfaceArray(slots: 1, seqLen: ctx1)
        vFull1 = try ioSurfaceArray(slots: 1, seqLen: ctx1)
        // Chunk2: 5 sliding + 2 full
        kSliding2 = try ioSurfaceArray(slots: 5, seqLen: w2)
        vSliding2 = try ioSurfaceArray(slots: 5, seqLen: w2)
        kFull2 = try ioSurfaceArray(slots: 2, seqLen: ctx2)
        vFull2 = try ioSurfaceArray(slots: 2, seqLen: ctx2)
    ...
    private func dummyPredict(model: MLModel, state: MLState) throws {
        let ctx = maskSequenceLength(for: model, inputName: "causal_mask") ?? contextLength
    ...
    func generate(messages: [ChatMessage], image: CGImage? = nil) async throws -> AsyncStream<String> {
        ...
        let promptMessages = budgetMessagesForContext(messages, hasImage: hasImage)
        let prompt = buildPrompt(messages: promptMessages, hasImage: hasImage)
        ...
        return AsyncStream { continuation in
            Task {
                ...
                do {
                    ...
                    let ctxLimit = self.runtimeContextLimit()
    ...
    private func predictChunked(tokenID: Int, position: Int, imageEmbedding: MLMultiArray? = nil) throws -> Int {
        ...
        let masks1 = try makeChunkMasks(for: chunk1, position: position)
        let masks2 = try makeChunkMasks(for: chunk2, position: position)
        let masks3 = try makeChunkMasks(for: chunk3, position: position)
        let masks4 = try makeChunkMasks(for: chunk4, position: position)
        ...
        let input1 = try MLDictionaryFeatureProvider(dictionary: [
            ...
            "causal_mask_full": MLFeatureValue(multiArray: masks1.full),
            "causal_mask_sliding": MLFeatureValue(multiArray: masks1.sliding),
            "update_mask": MLFeatureValue(multiArray: masks1.update),
        ...
        let input2 = try MLDictionaryFeatureProvider(dictionary: [
            ...
            "causal_mask_full": MLFeatureValue(multiArray: masks2.full),
            "causal_mask_sliding": MLFeatureValue(multiArray: masks2.sliding),
            "update_mask": MLFeatureValue(multiArray: masks2.update),
        ...
        let input3 = try MLDictionaryFeatureProvider(dictionary: [
            ...
            "causal_mask_full": MLFeatureValue(multiArray: masks3.full),
            "causal_mask_sliding": MLFeatureValue(multiArray: masks3.sliding),
            "update_mask": MLFeatureValue(multiArray: masks3.update),
            "per_layer_combined": MLFeatureValue(multiArray: plc),
            "cos_s": MLFeatureValue(multiArray: cosS),
            "sin_s": MLFeatureValue(multiArray: sinS),
            "cos_f": MLFeatureValue(multiArray: cosF),
        ...
        let input4 = try MLDictionaryFeatureProvider(dictionary: [
            ...
            "causal_mask_full": MLFeatureValue(multiArray: masks4.full),
            "causal_mask_sliding": MLFeatureValue(multiArray: masks4.sliding),
            "update_mask": MLFeatureValue(multiArray: masks4.update),
            "per_layer_combined": MLFeatureValue(multiArray: plc),
            "cos_s": MLFeatureValue(multiArray: cosS),
            "sin_s": MLFeatureValue(multiArray: sinS),
            "cos_f": MLFeatureValue(multiArray: cosF),
    ...
    private func runtimeContextLimit() -> Int {
        if isChunked {
            return chunkedDecodeContextLimit() ?? contextLength
        }
        if let model {
            return maskSequenceLength(for: model, inputName: "causal_mask") ?? contextLength
        }
    ...
    private func computePerLayerCombined(tokenID: Int, embedding: MLMultiArray) throws -> MLMultiArray {
        ...
        for i in 0..<hiddenSize { embF32[i] = Float(embF16[i]) }
    ...
    private func buildPrompt(messages: [ChatMessage], hasImage: Bool) -> String {
        ...
        for (i, m) in messages.enumerated() {
            if m.role == .user {
                let userContent = renderUserContent(m)
    ...
    private func budgetMessagesForContext(_ messages: [ChatMessage], hasImage: Bool) -> [ChatMessage] {
        guard let tokenizer else { return messages }

        let targetPromptTokens = max(128, runtimeContextLimit() - reservedCompletionTokens)
        let fullPrompt = buildPrompt(messages: messages, hasImage: hasImage)
        if tokenizer.encode(text: fullPrompt).count <= targetPromptTokens {
            return messages
        }
        ...
        for messageIndex in messages.indices.reversed() {
            let attachments = messages[messageIndex].attachments
            guard !attachments.isEmpty else { continue }
            for attachmentIndex in attachments.indices.reversed() {
                attachmentRefs.append(AttachmentRef(messageIndex: messageIndex, attachmentIndex: attachmentIndex))
                fitted[messageIndex].attachments[attachmentIndex].content = ""
            }
        }

        guard !attachmentRefs.isEmpty else { return messages }

        let basePrompt = buildPrompt(messages: fitted, hasImage: hasImage)
        let basePromptTokens = tokenizer.encode(text: basePrompt).count
        guard basePromptTokens < targetPromptTokens else { return fitted }

        var remainingAttachmentTokens = targetPromptTokens - basePromptTokens
        let trimNoticeTokens = tokenizer.encode(text: attachmentTrimNotice)

        for ref in attachmentRefs {
            guard remainingAttachmentTokens > 0 else { break }

            let originalContent = messages[ref.messageIndex].attachments[ref.attachmentIndex].content
                .trimmingCharacters(in: .whitespacesAndNewlines)
            guard !originalContent.isEmpty else { continue }

            let originalTokens = tokenizer.encode(text: originalContent)
            guard !originalTokens.isEmpty else { continue }

            if originalTokens.count <= remainingAttachmentTokens {
                fitted[ref.messageIndex].attachments[ref.attachmentIndex].content = originalContent
                remainingAttachmentTokens -= originalTokens.count
                continue
            }

            let availableContentTokens = max(0, remainingAttachmentTokens - trimNoticeTokens.count)
            guard availableContentTokens > 0 else { break }

            var trimmedContent = tokenizer.decode(tokens: Array(originalTokens.prefix(availableContentTokens)))
                .trimmingCharacters(in: .whitespacesAndNewlines)
            if trimmedContent.isEmpty { break }
            trimmedContent += attachmentTrimNotice
            fitted[ref.messageIndex].attachments[ref.attachmentIndex].content = trimmedContent
            remainingAttachmentTokens = 0
        }

        return enforcePromptBudget(fitted, hasImage: hasImage, promptBudget: targetPromptTokens)
    }

    private func enforcePromptBudget(_ messages: [ChatMessage], hasImage: Bool, promptBudget: Int) -> [ChatMessage] {
        guard let tokenizer else { return messages }

        var fitted = messages
        var promptTokens = tokenizer.encode(text: buildPrompt(messages: fitted, hasImage: hasImage)).count
        guard promptTokens > promptBudget else { return fitted }

        while promptTokens > promptBudget {
            guard let ref = attachmentRefForFurtherTrimming(in: fitted) else { break }

            let content = fitted[ref.messageIndex].attachments[ref.attachmentIndex].content
                .trimmingCharacters(in: .whitespacesAndNewlines)
            guard !content.isEmpty else { break }

            let tokens = tokenizer.encode(text: content)
            guard !tokens.isEmpty else {
                fitted[ref.messageIndex].attachments[ref.attachmentIndex].content = ""
                promptTokens = tokenizer.encode(text: buildPrompt(messages: fitted, hasImage: hasImage)).count
                continue
            }

            let reducedCount = max(0, tokens.count - min(64, tokens.count))
            if reducedCount == 0 {
                fitted[ref.messageIndex].attachments[ref.attachmentIndex].content = ""
            } else {
                fitted[ref.messageIndex].attachments[ref.attachmentIndex].content =
                    tokenizer.decode(tokens: Array(tokens.prefix(reducedCount)))
                    .trimmingCharacters(in: .whitespacesAndNewlines)
            }
    ...
    private func attachmentRefForFurtherTrimming(in messages: [ChatMessage]) -> (messageIndex: Int, attachmentIndex: Int)? {
        for messageIndex in messages.indices.reversed() {
            let attachments = messages[messageIndex].attachments
            guard !attachments.isEmpty else { continue }
            for attachmentIndex in attachments.indices.reversed() {
                let content = attachments[attachmentIndex].content
                    .trimmingCharacters(in: .whitespacesAndNewlines)
                if !content.isEmpty {
    ...
    private func renderUserContent(_ message: ChatMessage) -> String {
        let trimmed = message.content.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !message.attachments.isEmpty else { return trimmed }
        ...
        let attachmentText = message.attachments
            .map { attachment in
                ...
                case .email:
                    ...
                    prefix = "This is the latest visible email reply. Focus on the substantive message only. Ignore signatures, surveys, contact blocks, and tracking links.\n"
                case .text:
                    label = "Attached text file"
                    prefix = ""
                }
                return "[\(label): \(attachment.filename)]\n\(prefix)\(attachment.content)"
            }
            .joined(separator: "\n\n")

        if trimmed.isEmpty {
            return "Use the attached file content below.\n\n\(attachmentText)"
        }
