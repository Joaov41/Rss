...
final class ModelDownloader: NSObject {
    ...
    struct ModelInfo: Identifiable {
        ...
        static let defaults: [ModelInfo] = [
            ModelInfo(id: "gemma4-e2b", name: "Gemma 4 E2B", size: "2.5 GB",
                      ...
                      folderName: "gemma4-e2b-swa2048",
                      layoutPrefix: nil,
                      configRemotePath: "swa/model_config.json"),
    ...
    func localModelURL(for model: ModelInfo) -> URL? {
        ...
        if model.downloadURL.contains("huggingface.co"),
           !hasExpectedManifest(for: model, in: dir) {
            return nil
        }
        // Chunked
    ...
    func download(_ model: ModelInfo) async throws -> URL {
        ...
        if model.downloadURL.contains("huggingface.co"),
           fileManager.fileExists(atPath: destDir.path),
           !hasExpectedManifest(for: model, in: destDir) {
        ...
        try writeManifest(for: model, in: destDir)
    ...
    private func downloadFromHuggingFace(_ model: ModelInfo, to destDir: URL) async throws {
        ...
        // The `sdpa/swa` decode descriptors are internally inconsistent at
        // runtime (Core ML reports chunk1-3 as 8192 even though the bundled
        // metadata advertises 2048). The repo's top-level `swa/` + `prefill/`
        // layout matches the runner's existing interface and loads as a
        // consistent 2048-context set in Core ML.
        let layoutPrefix = model.layoutPrefix.map { "\($0)/" } ?? ""
        var files: [DownloadFile]

        // Download .mlmodelc files (same format as sdpa/ — verified working on iPhone).
        func mlc(_ remoteSubdir: String, _ remoteName: String, _ localName: String, weightSize: Int64) -> [DownloadFile] {
            [.init(remotePath: "\(layoutPrefix)\(remoteSubdir)/\(remoteName).mlmodelc/weights/weight.bin",
                   localPath: "\(localName).mlmodelc/weights/weight.bin", estimatedSize: weightSize),
             .init(remotePath: "\(layoutPrefix)\(remoteSubdir)/\(remoteName).mlmodelc/coremldata.bin",
                   localPath: "\(localName).mlmodelc/coremldata.bin", estimatedSize: 1_000),
             .init(remotePath: "\(layoutPrefix)\(remoteSubdir)/\(remoteName).mlmodelc/model.mil",
                   localPath: "\(localName).mlmodelc/model.mil", estimatedSize: 450_000),
             .init(remotePath: "\(layoutPrefix)\(remoteSubdir)/\(remoteName).mlmodelc/metadata.json",
                   localPath: "\(localName).mlmodelc/metadata.json", estimatedSize: 8_000),
             .init(remotePath: "\(layoutPrefix)\(remoteSubdir)/\(remoteName).mlmodelc/analytics/coremldata.bin",
        ...
        files += [
            .init(remotePath: model.configRemotePath ?? "\(layoutPrefix)model_config.json",
                  localPath: "model_config.json", estimatedSize: 1_000),
            // Tokenizer
            ...
            .init(remotePath: "swa/cos_sliding.npy", localPath: "cos_sliding.npy", estimatedSize: 524_416),
            .init(remotePath: "swa/sin_sliding.npy", localPath: "sin_sliding.npy", estimatedSize: 524_416),
            .init(remotePath: "swa/cos_full.npy", localPath: "cos_full.npy", estimatedSize: 1_048_704),
            .init(remotePath: "swa/sin_full.npy", localPath: "sin_full.npy", estimatedSize: 1_048_704),
        ...
        for file in files {
            ...
            // Resume: skip only if file size is close to expected.
            // This avoids mixed model sets when older chunks exist in the same folder.
            if fileManager.fileExists(atPath: destFile.path) {
                let attrs = try? fileManager.attributesOfItem(atPath: destFile.path)
                let existingSize = attrs?[.size] as? Int64 ?? 0
                if existingSize > 0 {
                    let tolerance = Swift.max(Int64(4_096), file.estimatedSize / 100)  // 1% or 4KB
                    if abs(existingSize - file.estimatedSize) <= tolerance {
    ...
    private func manifestURL(for model: ModelInfo, in directory: URL) -> URL {
        directory.appendingPathComponent(".download-manifest.json")
    }

    private func expectedManifest(for model: ModelInfo) -> DownloadManifest {
        DownloadManifest(
            formatVersion: 1,
            layoutPrefix: model.layoutPrefix,
            configRemotePath: model.configRemotePath
        )
    }

    private func hasExpectedManifest(for model: ModelInfo, in directory: URL) -> Bool {
        let url = manifestURL(for: model, in: directory)
        guard
        ...
        let expected = expectedManifest(for: model)
        return manifest.formatVersion == expected.formatVersion
            && manifest.layoutPrefix == expected.layoutPrefix
            && manifest.configRemotePath == expected.configRemotePath
    }

    private func writeManifest(for model: ModelInfo, in directory: URL) throws {
        let url = manifestURL(for: model, in: directory)
        let data = try JSONEncoder().encode(expectedManifest(for: model))
