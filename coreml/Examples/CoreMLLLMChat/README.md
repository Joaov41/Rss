...
Supports text chat (Qwen2.5, Gemma 4), `.txt` / `.md` / `.eml` / `.pdf` document attachments,
and image understanding (Gemma 4 multimodal).
...
- Chat with text
- Tap the paperclip icon to attach a `.txt`, `.md`, `.eml`, or `.pdf` document
...
- **Document input** — `.txt`, `.md`, `.eml`, and `.pdf` imports are extracted to prompt text
