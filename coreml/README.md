...
let summary = try await llm.generate(
    ...
    attachments: [
        PromptAttachment(filename: "notes.txt", content: "Meeting notes go here...")
    ]
)
...
`PromptAttachment` accepts plain text content, so app layers can extract `.txt`, `.md`, `.eml`, or `.pdf`
files and feed the text into the prompt without changing the inference engine.
