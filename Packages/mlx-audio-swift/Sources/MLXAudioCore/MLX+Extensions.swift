...
extension MLXArray {
    ...
    static func stack(_ arrays: [MLXArray], axis: Int = 0) -> MLXArray {
        ...
        case .float16:
            result = MLXArray.zeros(newShape, dtype: .float16)
        ...
        case .bfloat16:
            result = MLXArray.zeros(newShape, dtype: .bfloat16)
    ...
    static func arange(start: Float = 0.0, stop: Float, step: Float = 1.0, dtype: DType = .float32) -> MLXArray {
        ...
        guard (step > 0 && start < stop) || (step < 0 && start > stop) else {
            ...
            switch dtype {
            case .float32: return MLXArray.zeros([0], type: Float.self)
            case .float16: return MLXArray.zeros([0], dtype: .float16)
            case .bfloat16: return MLXArray.zeros([0], dtype: .bfloat16)
        ...
        case .bfloat16:
            let data = sequence.map { Float($0) }
            return MLXArray(data).asType(.bfloat16)
        default:
