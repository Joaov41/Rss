...
public struct GenerateParameters: Sendable {
    ...
    /// top k sampling; 0 means disabled
    public var topK: Int
    ...
    /// compatibility field for newer callers; currently not applied by this package revision
    ...
    public func sampler() -> LogitSampler {
        if temperature == 0 {
            ...
            return TopKSampler(temperature: temperature, topK: topK)
...
/// Sampler that restricts sampling to the top k logits.
public struct TopKSampler: LogitSampler {
    ...
    public func sample(logits: MLXArray) -> MLXArray {
        ...
        return withRandomState(randomState) {
            ...
            let vocabularySize = scaled.shape.last ?? topK
            let k = min(topK, vocabularySize)
            let topIndices = sortedIndices[0..., (-k)...]
            let topLogits = take(scaled, topIndices, axis: -1).squeezed(axis: 0)
            let sampled = categorical(topLogits)
            return topIndices.squeezed(axis: 0)[sampled]
        }
    }
}
...
/// ```
///
/// Tokens are integers that can be passed through a `Tokenizers.Tokenizer` or ``StreamingDetokenizer`` to produce Strings.
///
