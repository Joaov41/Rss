...
extension MLXRandom {
    ...
    static fileprivate let taskLocalRandomStateScope = ScopedValue<MLXRandom.RandomState?>(
        default: { nil })

    static fileprivate var taskLocalRandomState: MLXRandom.RandomState? {
        taskLocalRandomStateScope.current
    }
...
public func withRandomState<R>(_ state: MLXRandom.RandomState, body: () throws -> R) rethrows -> R {
    try MLXRandom.taskLocalRandomStateScope.withValue(state, operation: body)
}
...
public func withRandomState<R>(_ state: MLXRandom.RandomState, body: () async throws -> R)
    async rethrows -> R
{
    try await MLXRandom.taskLocalRandomStateScope.withValue(state, operation: body)
}
