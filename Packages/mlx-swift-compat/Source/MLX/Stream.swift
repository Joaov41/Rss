...
public final class Stream: @unchecked Sendable, Equatable {
    ...
    private static let defaultStreamScope = ScopedValue<Stream?>(default: { nil })

    static var defaultStream: Stream? {
        defaultStreamScope.current
    }
        ...
        rethrows -> R
    {
        ...
        return try defaultStreamScope.withValue(Stream(device), operation: body)
    }
    ...
    public static func withNewDefaultStream<R>(
        ...
        let device = device ?? Device.defaultDevice()
        return try await defaultStreamScope.withValue(Stream(device), operation: body)
    }
