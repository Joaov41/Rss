...
private final class ErrorHandler: @unchecked Sendable {
    ...
    private static let errorHandlerScope = ScopedValue<[@Sendable (String) -> Void]>(
        default: { [] })
    ...
    func dispatch(_ message: String) {
        if let handler = Self.errorHandlerScope.current.last {
            handler(message)
        } else {
            lock.withLock {
    ...
    public func withErrorHandler<R>(
        ...
        try ErrorHandler.errorHandlerScope.withValue(
            ErrorHandler.errorHandlerScope.current + [handler]
        ) {
            try body()
        }
    }
    ...
    public func withErrorHandler<R>(
        ...
        try await ErrorHandler.errorHandlerScope.withValue(
            ErrorHandler.errorHandlerScope.current + [handler]
        ) {
            try await body()
        }
